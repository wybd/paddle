...
# lenet-5网络模型 两个卷积池化相连+一个fc sortmax输出网络层
# 定义第一个卷积激活池化层
conv_rule_pool_1 = fluid.nets.simple_img_conv_pool(
    input=img,
    num_filters=10,  # 卷积核个数(对应类别)
    filter_size=5,  # 卷积核大小
    pool_size=2,  # 池化大小
    pool_stride=2,  # 池化步长
    act="relu",  # 激活函数
...
# 定义第二个卷积激活池化层
conv_rule_pool_2 = fluid.nets.simple_img_conv_pool(
    input=conv_rule_pool_1,  # 数据输入
    num_filters=5,  # 卷积核个数
    filter_size=2,  # 卷积核大小
    pool_size=2,  # 池化大小
    pool_stride=2,  # 池化步长
